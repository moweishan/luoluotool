...
**阶段目标**：接通真实键鼠通道，让占位任务 A 能按配置坐标完成一次真实点击序列。

**本次只做什么**：
1. `src/luoluotool/automation/input_sender.py`：ctypes `SendInput` 封装 `move_to/click/click_at/key_tap`；每个动作前检查 stop_event；动作间隔取自配置。
2. `src/luoluotool/config/models.py`：给 `placeholder_task_a.params` 定义结构 `{"click_points": [[x,y], ...], "wait_after_ms": 500}`（schema 仍为 v1，只填 params 内容）。
3. 干跑/真实分流：`TaskContext.dry_run == True` 时输入层被替换为“只写日志”；`False` 时走真实 SendInput。
4. 真实模式启动前弹出确认对话框：说明“将接管鼠标键盘，按 F8 可急停”，并提醒封号风险；用户确认后才进入真实模式。
5. 执行前检查：找到窗口并置前；`pause_on_window_focus_loss=true` 时，窗口失焦即暂停并提示，重新聚焦后继续。
6. 设置页把急停键、点击间隔改为**可编辑**（保存后生效）。
7. 测试：注入假 sender 验证点击序列与间隔、失焦暂停逻辑；单测永不产生真实输入。

**不要做什么**：不做图像识别；不再新增第二个任务；不做卡订单真实逻辑；不修改游戏进程；不绕过 UAC/权限。
...
- [ ] 真实模式（坐标先填桌面空白处验证）：确认框 → 点击序列按间隔执行 → 日志逐条记录 → F8 立即停止 → 游戏失焦时自动暂停。
...
SendInput 前检查 stop_event。禁止图像识别、禁止新增第二个任务、
