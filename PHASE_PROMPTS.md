...
>
> **改名注记（2026-09-22）**：原始 `README.md`（工程初始化手册）已改名为 `PROJECT_HANDBOOK.md`，
> `README.md` 换成了用户视角的使用说明；下文历史记录里出现的 `README.md` 一律指**工程手册**
> （即含「构建与发布」小节的那个文件）。
...
> **追加（2026-09-21，第四轮审查报告 `reports/CODE_REVIEW_REPORT_20260921_1.md` 的修复）**：
> 报告结论 `APPROVE` 附 5 条 P2 + 9 条 P3，本轮**全部处理完**：
> **P2-1（用户可见缺陷，先修）**「点一下再拖拽 → 拖出来的框被丢弃、反而留下看不见的 8×8 选区」——
> 三处一起改：松手丢弃退化选区、绘制矩形小于 `HANDLE_SIZE_PX` 时不给手柄、`hit_test` 只算一次手柄矩形，
> 并按评审的原始步骤（400×300 图 / 360×240 控件 / scale 0.8）离屏复现验证：修复后单击不再留残影、
> 随后拖拽得到 (40,68,60,57) 的正常选区。
> **P2-2**：试识别阈值改为**调试页当前值**（`debug_page.vision_threshold()` → 弹窗构造参数），
> 结论措辞限定为"本图 1:1 匹配（阈值 X.XX）下…"（不再写"识别不会认错"）。
> **P2-3**：跑的过程中改选区时丢弃旧结论并提示"已作废"。**P2-4**：关窗**不再阻塞 GUI** ——
> `request_stop()` + 只断开本弹窗的槽 + 线程由模块级 `ACTIVE_PROBES` 强引用到自然结束；
> 按钮按 `probe_in_flight()` 判定、槽里按 `self.sender()` 保护置空。**P2-5**：测试里的 git 调用统一加
> `-c core.quotePath=false`（并在强制 `quotePath=true` 下复跑验证）。
> **P3 一批**：删死常量与零调用 `set_zoom()`；1:1 改成双击滑条；滑条换算改以 2 为底的浮点倍数
> （101 个刻度逐点往返一致）；抽样判纯色时全分辨率复核；anchors 的 `.gitignore` 补 5 种后缀；
> 关于页诊断信息改相对路径 + 目录 tooltip 延迟 + 版本读取失败记日志 + 补"误操作风险"；
> `hit_test`/探针框绘制去重；三个框选测试的 `_image()` 收敛进 `tests/gui_helpers.py`。
