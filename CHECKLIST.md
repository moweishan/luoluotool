...
- [x] **日常任务页重建（用户设计稿，2026-09-21 起，分 3 批）**：**批 1 只做界面（已完成）**：照 `日常任务设计稿.html` 搭出「总开关与循环时长 / 关键建筑位置以及图像识别所需图片（鸡舍·土地·水产养殖）/ 产物制造」三组控件，控件名＝设计稿 id，动作按钮先禁用，**不写配置不置脏**（当时由守卫测试钉住，现已换成绑定测试）；**批 2 接配置 + 选择逻辑（已完成，2026-09-22）**：双向绑定（控件改动→写配置+置脏；`set_config()` 只显示、`_loading` 防回写）、`daily_tasks.enabled` **真的当总开关**（关掉→日常组不入队）、循环间隔界面分钟↔配置秒（复用 `loop.interval_seconds`，装载不回写）、`schema_version` 9→10 + 迁移（只补默认值；"老配置升级后原有字段一字不变"有测试）、`config.example.json` 同步、`--measure-layout` 仍「不改变任何高度」；「选择图片…」（默认 `assets/templates/`、5 种后缀、**选中即校验**：纯色拒收/低辨识度只提示）＋「截取游戏画面」＝**方案 2 框选**（先切游戏到前台 0.4s → 截客户区 → 复用框选弹窗 → 只存框选区域到 `assets/anchors/`，不入库）＋「选择的图片」等比缩放预览与双击放大（工具内弹窗）；失败只写日志 + 状态栏提示（含日志路径）、不弹窗、不动原有参考图。回归：`tests/test_gui_daily_page.py`（16 条）+ `tests/test_gui_daily_media.py`（21 条）+ `tests/test_gui_daily_capture.py`（12 条）+ `tests/test_paths.py` 的路径换算 3 条 + config/runner 层若干；**批 3 接执行**（按配置自动找建筑 → 执行真实日常任务动作）**尚未开始**。
  - **参考图可多选（2026-09-22 追加，已完成）**：三个建筑的参考图由单张字符串升级为 **`list[str]`**（**schema 10→11** + `_migrate_v10_to_v11` + 单测；`MAX_REFERENCE_IMAGES`＝10，validator 与界面同一份常量）。显示＝**缩略图列表 + 点选看大图**（`widgets.ThumbnailStrip`，点＝换大图、双击＝放大、右键＝移除这张 / 清空全部）；**「选择图片…」＝多选并替换整批**（`getOpenFileNames`，超 10 张只取前 10、重复去重、超长路径当场丢弃）；**「截取游戏画面」＝追加**（已在列表里不重复加、满 10 张拒绝并提示先移除一张）；输入框多张时显示「共 N 张：文件名…」，完整路径进 tooltip。回归：`tests/test_gui_daily_page.py` 的多选/移除/清空 3 条 + `tests/test_gui_daily_media.py` 的多选去重截断/混选/移除清空 4 条 + `tests/test_gui_daily_capture.py` 的追加与满额拒绝 2 条 + `tests/test_config` 的 v11 迁移与列表校验。
  - **可见的移除入口（2026-09-22 追加，已完成）**：每张缩略图右上角画 `×` 角标（悬停红底白叉 +「移除第 N 张」提示，点它**只删这一张、不动选中项**；角标上的双击不当作放大），缩略图条右侧加**可见的「清空全部」按钮**（id `{prefix}_clear_images`，没图时禁用），右键菜单保留。回归：`test_daily_page_thumbnail_has_a_visible_close_badge`、`test_daily_page_has_a_visible_clear_button_next_to_the_strip`、`test_daily_page_clear_button_sits_beside_the_thumbnail_strip`（几何钉子：按钮与缩略图条同行且在右边）。**删除时连真实图片一起删 + 二次确认（2026-09-22 追加，已完成）**：口径＝只真删 `assets/templates/` 与 `assets/anchors/` 里的图片（仓库外选的图只从列表移除）、「×」与「清空全部」都先弹一次确认（默认按钮＝取消）、还被别的建筑引用的文件不删；确认后删文件，取消则配置/界面/磁盘全不动；单个文件删失败照样移除条目，但状态栏写明原因与日志路径。**不可撤销**（templates 里入库的图片可用 git 恢复，anchors 的框选产物删了就没了）。代码：`gui/daily_files.py`（纯策略）、`gui/dialogs/confirm.py`（弹框）、`gui/daily_deletion.py`（mixin）；`tests/conftest.py` 新增 `_no_real_modal_dialogs` 夹具（忘了替换确认框＝立刻失败，不再卡死 600 秒）。回归：`tests/test_gui_daily_files.py`（7 条）+ `tests/test_gui_daily_delete.py`（8 条）。**暂时只读的开关**：「自动识别存量最少的产物并优先制造」按用户要求先做成只读，但它**仍是要入库的开关**（`data-key=auto_produce_least`、已绑配置、默认 False），只读由 `AUTO_PRODUCE_LEAST_READONLY` 一行控制（开放给用户＝改 False）。**两处错别字（用户 2026-09-21 确认改掉）**：「位于那个岛屿上」→「所在岛屿编号」、「自动识别那个产物少造那个」→「自动识别存量最少的产物并优先制造」；**设计稿 HTML 与代码同步改**，旧文案由 `test_the_two_design_typos_are_not_reintroduced` 钉住。**图片显示区（用户 2026-09-21 追加）**：每个建筑分组的最后一行是三列同行 —— 「截取…位置」/ **「选择的图片」显示区**（`{prefix}_selected_image`，批 2 已接上"选好的图在这里显示"）/「示例图片」；三块顶边对齐有几何测试钉住，**设计稿 HTML 已同步补上这一列**（`.two-col` → `.three-col`）。**口径来源**：用户填写的《日常任务待补逻辑清单》（仓库外 `D:\deepSeekHarness\日常任务待补逻辑清单.md`）。**后置（批 3 及以后）**：按配置找建筑（规则已定＝多尺度匹配 + 重试 2 次 + 跳过继续 + 失败截图存 `logs\failures\`）、真实日常任务动作序列、存量最少产物（用户要求"后续单独完成"）、每轮归位（TODO）、说明图真实图片。**失败处理**：只写日志并提示日志路径，**不发通知**。
...
- [ ] 杀软误报有应对说明（`PROJECT_HANDBOOK.md` §7.3 有加白名单步骤，README 的 FAQ 里有指路）。
- [ ] exe 启动耗时达标（≤ 5 秒）。

## 9. 备份与恢复

- [ ] 用户只需备份 `user_data/config.json` 即可迁移配置（README 的「配置」一节写明）。
...
- [ ] ⚠️ 若采用驱动级 / 注入类 / 过检测类 / 联网功能：必须有用户逐项授权记录、可回退的基础模式、`README.md`（风险声明一节）与「关于」页风险声明（见 `PROJECT_SPEC.md` §4.1）。
...
- [ ] ⚠️ **单步运行（调试，2026-09-22 用户要求）**：调试页最上方有「单步运行」开关（**不写配置**，未开启开发者调试时不生效并回滚）；主界面「停止」右边有「上一步 / 下一步」，**只有勾了单步才显示**；勾上后点「启动」＝每点一次「下一步」走一步（**一步＝一个动作**）；「上一步」＝指针退一格 + 光标回到那一步的位置、**绝不重放动作**；连点第二下「下一步」被忽略；关掉开关/急停/运行结束都唤醒等待者（不卡死）。代码：`core/step_mode.py`（控制器）、`gui/step_debug.py`（mixin + 跨线程桥）、`InputSender.cursor_position()` / `move_cursor()`、`TaskContext.step_gate()/step_done()`、`Runner(..., stepper=…)`。回归：`tests/test_core/test_step_mode.py`（14 条）+ `tests/test_gui_step_mode.py`（8 条）。
...
- [ ] ✅ 界面「关于」页与 `README.md`（风险声明一节）声明（2026-09-20 已实现「关于」页：`gui/pages/about.py`，页签「关于」排在最后）：自动化可能违反游戏条款，存在**封号风险**，**仅限个人学习自用、不发布不售卖、不提供防沉迷规避**；同时列出第三方组件与许可（PySide6 LGPL 动态链接、pywin32、numpy、opencv-python-headless）、数据与隐私（纯本地、不联网、不上传日志/配置/截图）、作者与仓库地址、运行环境与「复制诊断信息」/「打开数据目录」按钮。回归测试：`tests/test_gui_about.py`。
- [ ] ⚠️ 不提供未成年人防沉迷规避功能。
- [ ] ⚠️ 不发布、不售卖、不用于工作室多开打金；仓库 `README.md` 的「风险声明」写明。
