...
- [ ] **日常任务页重建（用户设计稿，2026-09-21）**：分 3 批 —— **批 1 只做界面（已完成）**：照 `日常任务设计稿.html` 搭出「总开关与循环时长 / 关键建筑位置以及图像识别所需图片（鸡舍·土地·水产养殖）/ 产物制造」三组控件，控件名＝设计稿 id，动作按钮先禁用，**不写配置不置脏**（守卫测试钉住）；**批 2 接配置**（双向绑定 + 非法回退 + `schema_version` +1 与迁移 + 单测）；**批 3 接执行**（选择图片/截取游戏画面/按配置自动找建筑）。**暂时只读的开关**：「自动识别存量最少的产物并优先制造」按用户要求先做成只读，但它**仍是要入库的开关**（`data-key=auto_produce_least`、第 2 批绑配置、默认 False），只读由 `AUTO_PRODUCE_LEAST_READONLY` 一行控制（开放给用户＝改 False）。**两处错别字（用户 2026-09-21 确认改掉）**：「位于那个岛屿上」→「所在岛屿编号」、「自动识别那个产物少造那个」→「自动识别存量最少的产物并优先制造」；**设计稿 HTML 与代码同步改**，旧文案由 `test_the_two_design_typos_are_not_reintroduced` 钉住。**图片显示区（用户 2026-09-21 追加）**：每个建筑分组的最后一行是三列同行 —— 「截取…位置」/ **「选择的图片」显示区**（`{prefix}_selected_image`，第 2 批接上"选好的图在这里显示"）/「示例图片」；三块顶边对齐有几何测试钉住，**设计稿 HTML 已同步补上这一列**（`.two-col` → `.three-col`）。回归：`tests/test_gui_daily_page.py`（12 条：控件名＝设计稿 id 的契约测试、旧文案不回来、只读开关"仍可入库"、三块图片区同行、动作按钮禁用、"不写配置"守卫）。**批 2 口径已由用户填写确认（2026-09-21，填写清单见仓库外 `D:\deepSeekHarness\日常任务待补逻辑清单.md`）**：本批＝**配置绑定 + 选择逻辑**（`daily_enabled` 复用 `features.daily_tasks.enabled` 并真的当总开关；循环间隔走"复用 `loop.interval_seconds`、界面按分钟换算"；单岛屿 int 1–10；参考图存相对路径；`auto_produce_least` 默认 false 且**继续只读**；字段平铺 `features.daily_tasks.<data-key>`；schema 9→10 +"老配置升级后原有字段一字不变"测试）；**选择逻辑**＝「选择图片…」（默认目录 `assets/templates/`、5 种后缀、选中即校验能否识别）+「截取游戏画面」＝**方案 2 框选**（自动把游戏切前台约 0.4 秒 → 截客户区 → 弹现有框选窗 → 存 `assets/anchors/`，不入库）+「选择的图片」等比缩放预览与双击放大；**后置**：按配置找建筑（规则已定＝多尺度匹配 + 重试 2 次 + 跳过继续 + 失败截图存 `logs\failures\`）、真实日常任务动作序列、存量最少产物（用户要求"后续单独完成"）、每轮归位（TODO）、说明图真实图片。**失败处理**：只写日志并提示日志路径，**不发通知**。
