...
- [ ] **日常任务页重建（用户设计稿，2026-09-21）**：分 3 批 —— **批 1 只做界面（已完成）**：照 `日常任务设计稿.html` 搭出「总开关与循环时长 / 关键建筑位置以及图像识别所需图片（鸡舍·土地·水产养殖）/ 产物制造」三组控件，控件名＝设计稿 id，动作按钮先禁用，**不写配置不置脏**（守卫测试钉住）；**批 2 接配置**（双向绑定 + 非法回退 + `schema_version` +1 与迁移 + 单测）；**批 3 接执行**（选择图片/截取游戏画面/按配置自动找建筑）。**只读项**：「自动识别那个产物少造那个」按用户要求做成只读（文案带 `（只读）`、点不动、不灰掉、程序仍可设置）。回归：`tests/test_gui_daily_page.py`（9 条）。
