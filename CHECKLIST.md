...
- [ ] ⚠️ 输入实现方式合规（2026-09-15 更新）：窗口消息发到游戏窗口/子窗口；若用**合成指针**（`InjectSyntheticPointerInput`/`InjectTouchInput`）：坐标经 `ClientToScreen` 换算、`hwndTarget` 指向目标窗口、注入前校验前台、绝不移动真实光标、逐条日志；单测仍须注入假 `user32`、零真实注入。
