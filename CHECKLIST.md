...
- [ ] ⚠️ 对齐窗口点击通道（Phase 5.2）合规：`automation/window_align.py` 内不得出现 `SetCursorPos` / `mouse_event` / `SendInput` / 合成指针注入；点击前等光标静止（≤50 px/s）、最大化窗口拒绝点击、`finally` 还原窗口位置并记录对齐误差与还原结果。
...
- [ ] ⚠️ 输入方式三档合规（`automation.input_mode`）：① `window_message` 不接管键鼠；② `window_align` 只移动窗口、不动光标（`window_align.py` 内不得出现移动光标的 API）；③ `real_input` 走 `SendInput` 会移动光标并抢前台——必须做到**每次点击/按键前**校验并置顶/置前、无法确保在最前时绝不输入、`finally` 还原光标并按需取消本次置顶；注入 API 只允许出现在 `automation/real_input.py`（有守卫测试）。
