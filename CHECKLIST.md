...
- [ ] ⚠️ 多尺度识别：识别默认做缩放搜索（**0.4x–4.0x**，`Match.scale` 报出比例）；**阈值必须在精修之后判断**；精修只接受严格更好的分数；结束粗搜必须是**"越过峰值回落"**（禁止"第一个够强的候选就停"——实测真值 3.50x 会被截断成 3.36x）；终止粗搜还要求最佳候选面积 ≥ 原模板 30%（防小缩放虚高分）；`--no-scale` 可退回原始尺寸。
