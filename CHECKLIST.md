...
- [ ] **日常任务页重建（用户设计稿，2026-09-21）**：分 3 批 —— **批 1 只做界面（已完成）**：照 `日常任务设计稿.html` 搭出「总开关与循环时长 / 关键建筑位置以及图像识别所需图片（鸡舍·土地·水产养殖）/ 产物制造」三组控件，控件名＝设计稿 id，动作按钮先禁用，**不写配置不置脏**（守卫测试钉住）；**批 2 接配置**（双向绑定 + 非法回退 + `schema_version` +1 与迁移 + 单测）；**批 3 接执行**（选择图片/截取游戏画面/按配置自动找建筑）。**暂时只读的开关**：「自动识别那个产物少造那个」按用户要求先做成只读，但它**仍是要入库的开关**（`data-key=auto_produce_least`、第 2 批绑配置、默认 False），只读由 `AUTO_PRODUCE_LEAST_READONLY` 一行控制（开放给用户＝改 False）。**图片显示区（用户 2026-09-21 追加）**：每个建筑分组的最后一行是三列同行 —— 「截取…位置」/ **「选择的图片」显示区**（`{prefix}_selected_image`，设计稿 HTML 里还没有，第 2 批接上"选好的图在这里显示"）/「示例图片」；三块顶边对齐有几何测试钉住，改设计稿时把那一行补成三列。回归：`tests/test_gui_daily_page.py`（11 条：控件名＝设计稿 id 的契约测试、只读开关"仍可入库"、三块图片区同行、动作按钮禁用、"不写配置"守卫）。
