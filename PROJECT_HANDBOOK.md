...
| 单步运行（2026-09-22） | 调试页最上方新增「单步运行」开关；勾上后点「启动」= **每点一次「下一步」脚本才走一步**（一步＝一个动作），主界面「停止」右边出现「上一步 / 下一步」（不勾不显示）；「上一步」只把步骤指针退回一格并把光标移回那一步的位置、**不重放动作**；开关不存盘 |
...
| 日常任务页（2026-09-22 接配置） | 按设计稿重建，**第 2 批已接上配置与"选择逻辑"**：**总开关与循环时长**（启用日常任务 / **按固定间隔循环** / 循环间隔分钟 —— 三项都存盘，循环间隔界面按分钟、配置里存秒，只有你改那个框才写回）、**关键建筑位置以及图像识别所需图片**（鸡舍 / 土地 / 水产养殖各一组：所在岛屿编号 + **参考图（可多选，最多 10 张）** + 「选择的图片」大图 + 缩略图列表 + 示例图 —— 岛屿与路径都存盘）、**产物制造**（自动识别产物最少的优先造，**该项暂为只读**，值照常入库）。**按钮现在真的能用**：「**选择图片…**」默认打开 `assets/templates/`（只收 png/jpg/jpeg/bmp/webp，选中即校验 —— 纯色图会被拒收并提示换一张）**且可以一次选多张**（最多 10 张，重复的自动去掉、超出的只取前 10 张并提示；选中的这几张会**替换**原来那一组）；「**截取游戏画面**」＝先抢约 0.4 秒把游戏切到前台 → 截客户区 → 弹出框选窗口 → **只保存你框住的那块**到 `assets/anchors/`（文件名如 `鸡舍_岛屿1_<时间戳>.png`）并**追加**到已选图片末尾（满 10 张要先在缩略图上右键移除一张）；**点缩略图＝看那张大图、双击＝放大、点右上角「×」＝删掉那一张**（悬停会提示是第几张）、**缩略图条右边的「清空全部」按钮＝一次清掉这一组**（没图时按钮是灰的）、右键＝同样两条（移除 / 清空）。**删除会连磁盘上的图片一起删**：先弹一次确认（默认按钮是「取消」），确认后只删 `assets/templates/` 与 `assets/anchors/` 里的文件，仓库外的图片（比如从桌面选的）只从列表移除；同一张图还被别的建筑用到时也不会删文件。删除不可撤销 —— `assets/templates` 里入库的图片可以用 git 恢复，`assets/anchors` 里的框选产物不入库、删了就没了。参考图路径按**相对仓库根**存，图片被删/改名只提示、不改配置。**尚未实现**：按配置自动去找三个建筑、以及真正的日常任务动作流程（第 3 批） |
