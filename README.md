...
| 框选弹窗怎么用（2026-09-20 增强） | 拖出选区后**还能改**：拖选区内部＝移动、拖四角/四边小方块＝改大小（按住 **Alt** ＝以中心对称缩放）、**空格/右键拖拽**＝移动、选区外重拖＝重新框选、**Esc**＝撤销这次拖拽（没拖时清空）、**双击**＝清空重来；**方向键**微调 1 像素（Shift＝10 像素，Ctrl＝对应那条边外扩 1 像素，Ctrl+Shift＝内收 1 像素）。**看不清就缩放**：**滚轮**＝以鼠标为中心缩放、**中键拖拽**或**空格+拖拽**＝平移画面、「**适配窗口**」＝回到整图、「**1:1 显示**」＝一个图像像素对一个屏幕像素；鼠标旁常驻**放大镜**（6 倍整数放大 + 十字定位 + 坐标），信息行同时显示「选区尺寸 / 缩放倍率 / 鼠标客户区坐标」。框中的区域**外圈压暗**，拖拽时旁边显示「宽×高 @ 坐标」气泡。 |
