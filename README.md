...
| 图像识别 | **OpenCV 模板匹配**（`opencv-python-headless` + `numpy`，2026-09-19 引入）：给一张模板图 → 在游戏窗口客户区里找 → 返回命中坐标（中心/左上/尺寸/匹配度/**缩放比例**）。**画面放大或缩小都能识别**（多尺度匹配默认开启：先搜 0.3x–2.0x，找不到再扩到 0.3x–4.0x；`--no-scale` 可关闭）；PrintWindow 取到黑帧时自动换其它取景方式。用法：调试页「图片识别匹配测试」（可先点「框选截图生成模板」直接框选生成模板，会自动填入路径；「识别成功时保存带框截图」为可选开关），或命令行 `python -m luoluotool --recognize 图片.png --threshold 0.85`（退出码 0 命中 / 1 未命中）。模板存 `assets/anchors/`（不入库），带框截图存 `user_data/debug/vision_*.png` |
| 开发者调试 | 设置页可勾选「开发者调试」，顶部出现调试页：窗口诊断、布局测量、**图片识别匹配测试（放在页面最前：选图片 + 阈值 → 给出命中坐标）**、干跑开关、鼠标单点/连点、屏幕滑动、键盘输入四个带参数的测试按钮（复用同一输入通道与安全规则） |
