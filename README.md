...
| 日常任务页（2026-09-22 接配置） | 按设计稿重建，**第 2 批已接上配置与"选择逻辑"**：**总开关与循环时长**（启用日常任务 / **按固定间隔循环** / 循环间隔分钟 —— 三项都存盘，循环间隔界面按分钟、配置里存秒，只有你改那个框才写回）、**关键建筑位置以及图像识别所需图片**（鸡舍 / 土地 / 水产养殖各一组：所在岛屿编号 + 参考图路径 + 「选择的图片」预览 + 示例图 —— 岛屿与路径都存盘）、**产物制造**（自动识别产物最少的优先造，**该项暂为只读**，值照常入库）。**按钮现在真的能用**：「**选择图片…**」默认打开 `assets/templates/`（只收 png/jpg/jpeg/bmp/webp，选中即校验 —— 纯色图会被拒收并提示换一张）；「**截取游戏画面**」＝先抢约 0.4 秒把游戏切到前台 → 截客户区 → 弹出框选窗口 → **只保存你框住的那块**到 `assets/anchors/`（文件名如 `鸡舍_岛屿1_<时间戳>.png`）；双击「选择的图片」＝放大预览。参考图路径按**相对仓库根**存，图片被删/改名只提示、不改配置。**尚未实现**：按配置自动去找三个建筑、以及真正的日常任务动作流程（第 3 批） |
| 任务编排 | 队列 = **日常任务组**（**需总开关 `daily_tasks.enabled` 为真** + 任务勾选，按 `order` + 任务 ID 字典序）→ **单功能组**（功能主开关，固定 `order_hold` → `feature_3` → `feature_4`）；失败计数与循环对整队列统一；启动时日志打印 `任务队列（N 个）：a → b → c`（Phase 6；两个预留开关不参与编排；总开关关着却有任务勾选时会警告） |
| 框选弹窗怎么用（2026-09-20 增强） | 拖出选区后**还能改**：拖选区内部＝移动、拖四角/四边小方块＝改大小（按住 **Alt** ＝以中心对称缩放）、**空格/右键拖拽**＝移动、选区外重拖＝重新框选、**Esc**＝撤销这次拖拽（没拖时清空）、**双击**＝清空重来；**方向键**微调 1 像素（Shift＝10 像素，Ctrl＝对应那条边外扩 1 像素，Ctrl+Shift＝内收 1 像素）。**看不清就缩放**：**滚轮**＝以鼠标为中心缩放、**中键拖拽**或**空格+拖拽**＝平移画面，下面的**滑条**＝左右拉改缩放（100%＝整图适配，每往右 1/4 行程翻一倍，最高 800%；滚轮缩放时滑条会跟着走），旁边的「**重置**」＝一键回到初始状态（缩放回整图适配 + 清空选区）；鼠标旁常驻**放大镜**（6 倍整数放大 + 十字定位 + 坐标），信息行同时显示「选区尺寸 / **可辨识度** / 缩放倍率 / 鼠标客户区坐标」。框中的区域**外圈压暗**，拖拽时旁边显示「宽×高 @ 坐标」气泡。**保存前会查可辨识度**：选区几乎是纯色时不写文件、不关窗口，直接告诉你换一块有纹理/数字的区域（那种模板存下来只会在画面上到处误匹配，"偏低"时只提示、仍可保存）。**拿不准就点「在本图试识别」**：把当前选区当模板，当场在这张截图上匹配一次 —— "本图只命中你框的这一处"＝这块独一无二，放心存；"本图共 3 处相似，除你框的还有 2 处：(x, y)、(x, y)"＝识别时可能选错地方，那几处会直接标在图上（橙框 + 序号）。 |
