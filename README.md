...
| 安全底线 | 干跑模式默认开启 + F8 全局急停热键；禁止读写游戏内存、封包拦截/伪造；驱动级/注入类方案须逐项授权 + 风险声明（2026-09-15 政策变更） |
...
- 不读取/修改游戏内存，不拦截网络封包；
- 驱动级 / 注入类 / 过检测类方案必须**逐项授权**并声明风险（可能导致游戏无法启动、需改测试签名/Secure Boot、封号概率显著上升），详见 `PROJECT_SPEC.md` §4.1。

以上红线同时写入 `PROJECT_SPEC.md` 与 `AGENTS.md`，AI 必须在所有阶段遵守。
（2026-09-15 按用户要求取消了其中「DLL 注入 / 驱动级操作 / 过检测技巧」三条限制。）
