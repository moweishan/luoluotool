...
| 图像识别 | **OpenCV 模板匹配**（`opencv-python-headless` + `numpy`，2026-09-19 引入）：给一张模板图 → 在游戏窗口客户区里找 → 返回命中坐标（中心/左上/尺寸/匹配度）。用法：调试页「图片识别匹配测试」（可先点「框选截图生成模板」直接框选生成模板，会自动填入路径），或命令行 `python -m luoluotool --recognize 图片.png --threshold 0.85`（退出码 0 命中 / 1 未命中）。模板存 `assets/anchors/`（不入库），带框截图存 `user_data/debug/vision_*.png` |
