...
| 日常任务页（2026-09-21 改版） | 按设计稿重建：**总开关与循环时长**（启用登录/循环间隔分钟）、**关键建筑位置以及图像识别所需图片**（鸡舍 / 土地 / 水产养殖各一组：选岛屿编号 + 选/截参考图 + 示例图）、**产物制造**（自动识别产物最少的优先造）。**当前只做界面**：控件不写配置、「选择图片…」「截取游戏画面」按钮暂不可用（第 2/3 批接入） |
