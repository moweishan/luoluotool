...
5. **UI 与业务分离**：GUI 文件里不允许出现任务流程、SendInput、文件读写等逻辑；只允许出现事件绑定与展示。
6. **默认安全**：一切自动化默认 dry-run；任何真实键鼠输入路径都必须经过显式用户确认 + 急停可中断。
...
- 每次真实键鼠动作前检查 stop 事件；停止请求发出后 500ms 内必须停止动作序列。
- 文件长度控制：单个文件超过 400 行必须先考虑拆分；超过 600 行必须拆分（模板生成的 UI 文件除外）。

## 3. 禁止事项（红线）

1. 禁止读写游戏进程内存、DLL 注入、驱动、封包拦截/伪造——**连接口都不得预留**。
...
10. 禁止在游戏窗口未找到、未置前时执行点击；失焦行为必须遵守 `pause_on_window_focus_loss` 配置。
...
- 必须覆盖的测试类型：
  ...
  - automation：用注入的假 `send_input` 验证调用序列（**单测永不产生真实键鼠动作**）；
...
- Conventional Commits，中文说明，范围前缀必带：
  ...
  - `test(automation): 补充 SendInput 调用序列断言`
