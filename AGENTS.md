...
- 输入注入实现方式：窗口消息发到游戏窗口或其子窗口；若使用**对齐窗口点击**（`automation/window_align.py`），必须：只移动游戏窗口、绝不移动真实光标、源码中不得出现移动光标的 API（`SetCursorPos`/`mouse_event`/`SendInput`/合成指针注入）、点击前等真实光标静止（速度阈值 50 px/s）、最大化窗口直接拒绝点击（不得退化成"按光标乱点"）、无论成功失败都要在 `finally` 里还原窗口位置、逐条记录对齐误差与还原结果；若使用用户态合成输入，必须与游戏权限对齐、注入前校验目标窗口在前台、动作后还原真实光标（可配置）、逐条记录坐标/按键日志。
