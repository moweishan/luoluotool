...
- **多尺度匹配（2026-09-19 用户实测 bug：画面放大/缩小就识别不到）**：识别必须默认做缩放搜索（`locate_all_scaled`，0.4x–2.0x）：粗搜比例 → 在最佳比例附近精修（步长 0.02）→ 在该比例下取全部命中，`Match.scale` 记录所用比例。三条硬约束：① **阈值必须在精修之后再判断**（真实缩放常落在粗搜两档之间：实测 0.75x 在粗搜只有 0.83、精修到 0.74x 是 0.95）；② 精修阶段只接受**严格更好**的分数（不得再套"贴近 1.0x"的平局规则，否则会把 1.40x 掰成 1.38x）；③ 提前结束粗搜必须同时满足"分数 ≥ 阈值+0.05"**且**"候选模板面积 ≥ 原模板的 30%"（过小的缩放会给出虚高分数：实测 9×4 像素匹配到 0.96）。CLI `--no-scale` 可退回只按原始尺寸匹配。
