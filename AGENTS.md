...
6. **默认安全**：一切自动化默认 dry-run；任何真实输入注入路径都必须经过显式用户确认（询问频率可配置，默认每次）+ 急停可中断。输入实现方式可选：窗口消息（默认）／用户态合成输入（`SendInput`、`SetCursorPos`，须校验前台并还原光标）／驱动级注入（须逐项授权，见 `PROJECT_SPEC.md` §4.1）。
...
- 输入注入实现方式：窗口消息发到游戏窗口或其子窗口；若使用用户态合成输入（含**合成指针**：`CreateSyntheticPointerDevice` + `InjectSyntheticPointerInput`），必须：客户区坐标先经 `ClientToScreen` 换算、`hwndTarget` 指向目标窗口、注入前校验目标窗口在前台、**绝不移动真实光标**、逐条记录日志。
