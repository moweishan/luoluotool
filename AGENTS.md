...
- 点击越界硬规则（2026-09-19 用户要求）：**任何鼠标点击在发出前都必须校验坐标落在游戏窗口客户区内**（`input_sender.point_in_client_area`，客户区为 `[0,width)×[0,height)`）；越界或读不到客户区时**不点击**，只写 WARNING 日志（"点击坐标 (x, y) 不在游戏窗口内（客户区 WxH），已跳过本次点击"）。真实通道（`RealInputSender.click_at`）与干跑通道（`DryRunSender` 注入 `bounds` 回调）都要校验；客户区读取失败按越界处理（宁可不点击）。滑动（drag）当前**不在**该校验范围内。
