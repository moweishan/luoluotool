...
| 联网 | 当前**没有任何联网代码**。**计划中（未实现）**：软件启动时从远程服务器加载/更新配置项（预留缺口与硬约束见 §4.4）。**通知 / 推送（企业微信等）不作为软件功能**（§4 第 9 条）。无 LLM、无数据库、无定时任务（无需后台常驻调度） |
...
1. **日常任务组**：**总开关 `features.daily_tasks.enabled` 为真**（2026-09-22 起它真的当总开关，
   见 §9；同日起这一条取代了原先"只存/读/显示、不参与编排"的写法）**且**
   `features.daily_tasks.tasks[id].enabled == true` 的任务入队，按 `order` 升序；
   同一 `order` 时按任务 ID 字典序（保证每次运行顺序完全一致）。
   总开关关闭时**日常任务组一个任务都不入队，单功能组不受影响**（各有自己的开关）；
   若"总开关关着、却有任务被勾选"，`Runner` 记一条 WARNING 列出这些任务，主窗口启动时
   把同一句话写进日志面板（第五轮评审 P2-1：老配置升级后不能静默不跑）。
...
5. 循环：`features.daily_tasks.loop` 对**整队列**生效（开关 + 间隔秒数）；队列为空时不执行任何任务并立即结束。
   **总开关不代替循环开关**：日常任务页上有独立的「按固定间隔循环」复选框（绑 `loop.enabled`），
   不勾就只跑一轮（第五轮评审 P2-2：没有这个开关时"循环间隔"是失效控件）。
6. **仍然不参与编排**的开关：`order_hold.reserved_switch_1/2`（预留开关，保持「存/读/显示」语义）
   ——有测试断言它们不影响运行任务集合。（`features.daily_tasks.enabled` 已自 2026-09-22 起参与编排，见第 1 条。）
...
9. **禁止把「通知 / 推送」做成软件功能**（用户 2026-09-21 明确）：企业微信（以及任何 IM / 邮件 / webhook / 短信）通知**只属于开发过程的通知手段** —— 当前是开发代理在任务完成后用 `~/.dsh/push/notify.ps1` 发给用户，**与 LuoLuoTool 这个软件无关**。程序内**不得**出现通知、推送、上报相关代码，**不得**为它预留开关、接口、依赖或配置字段。（**例外见 §4.4**：唯一预留的联网用途是"启动时从远程服务器加载/更新配置项"。）
...
| **禁止任何联网代码** | 本文件原 §4 第 5 条；`AGENTS.md` 原 §3 红线 2 | **已取消**（数据保护约束保留）。**程序当前仍无联网代码**；唯一预留的联网用途是"启动时从远程服务器加载/更新配置项"（缺口与七条硬约束见 §4.4；**通知/推送不得做成软件功能**，见 §4 第 9 条） |
| **禁止绕过「真实模式确认」** | 本文件原 §4 第 7 条；`AGENTS.md` 红线 3 | **改为可配置**（默认每次询问，不再作为绝对禁令） |

取消后**仍然必须遵守**的约束：

- **逐项授权**：驱动级 / 注入类 / 过检测类 / 联网功能，必须先写入对应阶段的「本次只做什么」并取得用户明确确认，**禁止静默引入**（**预留的联网缺口**＝§4.4 的"启动时从远程服务器加载/更新配置项"，用户 2026-09-21 已确认保留该缺口，真做时仍走一遍本节流程）；
...
### 4.4 预留的联网缺口：启动时从远程服务器加载 / 更新配置项（计划中，未实现）
...
1. **只拉取、不上传**：只允许从服务器**下载配置项 / 配置模板**；**禁止上传**日志、配置、截图、设备指纹或任何本地数据（沿用 §4 第 4 条与 §5 的数据保护）。
2. **不可用必须降级**：网络不可用、超时、服务器报错、返回内容非法时，一律**退回本地配置**并保持程序可用 —— **绝不允许**因此启动失败、卡住或崩溃（**本地配置优先，远程只是覆盖**）。
3. **先校验后落盘**：远程内容必须走现有 `config/validation.py` 的校验与 schema 版本迁移；不合法就丢弃并记 WARNING；远程 `schema_version` 比程序新时**只读**（沿用 §5 与 `AGENTS.md` 的配置安全条款）。
4. **用户可见 + 默认关闭**：必须有界面开关（**默认关闭**）、必须显示"当前配置来自远程"的来源提示、每次联网都要记日志（URL、耗时、结果）。
5. **不静默改写用户配置**：远程配置落盘前按既有流程备份（`config.json.bak-v<版本>-<时间戳>`），且只在开关打开 / 用户同意时生效。
6. **依赖先批准**：优先用标准库（`urllib` / `http.client`）；若需第三方 HTTP 客户端，**先写进 `requirements.txt` 并说明体积代价**（`AGENTS.md` §1.8），不得"只在本机生效"。
7. **验收不放松**：实现那条功能时照旧跑全量验收（`pytest -q` / `--validate-config` / `--smoke-gui` / `--measure-layout` / 文档索引自检），并补"离线/坏响应/非法内容都能降级"的测试。

### 边界内但暂缓（不在 MVP）
- **启动时从远程服务器加载 / 更新配置项**：用户 2026-09-21 确认为**计划功能**（缺口与七条硬约束见 §4.4）—— **未实现**。
...
- **数据保护（用户明确保留）**：不得收集或存储账号、密码、token、设备指纹；联网功能不得上传日志、配置、截图等本地数据。**通知 / 推送不得作为软件功能**（§4 第 9 条）；唯一预留的联网用途见 §4.4（启动时远程加载/更新配置项，未实现）。
...
│   │   ├── dialogs/         # crop_dialog.py（弹窗外壳：提示/缩放滑条+重置/保存）
...
│       └── paths.py         # 运行时路径解析（user_data/logs/debug + assets 下的 templates/screenshots/anchors）
...
| core | 任务协议、注册表、执行调度、运行状态、开发者调试动作、识别编排（含**框选自检**） | `class BaseTask: run(ctx)`, `TaskRegistry.get(task_id)`, `Runner.start(config)`, `Runner.stop()`; `debug.run_single_click/repeat_click/swipe/key(...)`（单点/连点支持 `hold_ms`，长等待切片可中断）; `state.try_transition(...)`（非法迁移返回 False，不抛异常；状态读写加锁）; `vision.probe_region_on_image(image, region, *, threshold, max_results) -> TemplateProbeResult`（D1 框选自检：把选区当模板在同一张图上 1:1 试匹配，`duplicates`＝除自己以外的命中数；越界/太小抛 `VisionError`） |
...
| gui（参考图的删除策略，2026-09-22 新增） | 移除/清空参考图时**连磁盘文件一起删**（用户要求）的策略与执行：只真删 `assets/templates/` 与 `assets/anchors/` 里的图片、仓库外的只从列表移除、还被别的建筑引用的不删；弹框由 `dialogs/confirm.py` 负责（默认按钮＝取消） | `daily_files.managed_roots()` / `daily_files.plan_deletions(values, *, still_referenced) -> DeletionPlan`（`.deletable` / `.kept_outside` / `.kept_shared` / `.missing`、`.will_delete()` / `.names()` / `.describe()`）/ `daily_files.delete_files(plan, log) -> DeletionOutcome`（`.deleted` / `.failed` / `.summary()`）/ `daily_files.remove_file(path)`；`dialogs.confirm.confirm_destructive(parent, title, text, *, detail="", confirm_text="删除") -> bool` |
| core（单步运行/调试，2026-09-22 新增） | 「点一次「下一步」走一步」的单步控制器：执行线程在每步前等门，界面按「下一步」放行 / 按「上一步」把指针退回一格并给出"那一步的光标位置"（**不重放动作**）；非单步时空操作 | `step_mode.StepController(*, on_change=None)`：`.enabled()` / `.set_enabled(v)` / `.begin_run(initial_cursor=None)` / `.end_run()` / `.gate(index, total, label, *, should_stop) -> (StepDecision, 回位目标)` / `.complete(index, cursor)` / `.index()` / `.request_next() -> bool` / `.request_previous() -> (index, 回位目标)` / `.snapshot() -> StepSnapshot`；`StepDecision.RUN / REWOUND / STOPPED`；`TaskContext.stepper` + `.step_gate()` / `.step_done()`；`Runner(config, ..., stepper=None)` |
| gui（单步运行的界面接线，2026-09-22 拆出） | 调试页最上方的「单步运行」开关（`DebugPage.step_mode_box` / 信号 `step_mode_toggled(bool)` / `set_step_mode()` / `step_mode_enabled()`，**不写配置**）+ 主界面「停止」右边的「上一步 / 下一步」（只有勾了单步才显示）+ 执行线程→GUI 线程的状态桥 | `step_debug.StepDebugMixin`：`.build_step_controls()` / `.add_step_buttons(layout)` / `.reset_step_mode()` / `.attach_stepper_to_runner(runner)`；`step_debug.StepModeBridge.state_changed(object)`；`automation.input_sender.InputSender` 新增 `cursor_position() -> tuple \| None` 与 `move_cursor(x, y)` （干跑只记日志、只移动不点击） |
| gui（日常任务页的三块，2026-09-22 拆出） | `pages/daily_fields.py`＝叶子模块（`BUILDINGS` 与 `building_title` / `island_field` / `ref_image_field`，供页面/控制器/测试共用，避免循环 import）；`pages/daily_images.py`＝`DailyImagesMixin`（参考图的**显示与交互**：多选缩略图条 + 大图 + 移除/清空，**不碰文件、不写配置**）；`daily_deletion.py`＝`DailyDeletionMixin`（移除/清空＝先二次确认 → 连磁盘文件一起删 → 改配置与界面，2026-09-22 从 `daily_media.py` 拆出）；`daily_workers.py`＝解码/截图线程与结果类型（见上一行） | `DailyImagesMixin.set_reference_images(prefix, paths, images)` / `.set_reference_thumbnail(prefix, index, image)` / `.append_reference_image(prefix, path, image)` / `.remove_reference_image(prefix, index)` / `.clear_reference_images(prefix)` / `.reference_images(prefix)` / `.selected_index(prefix)` / `.preview_widget(prefix)` / `.thumbnail_strip(prefix)`；`widgets.ThumbnailStrip(empty_text="")`（横排缩略图，`image_selected(int)` / `image_activated(int)` / `remove_requested(int)` / `clear_requested()`，`set_images` / `set_image_at` / `count` / `select` / `selected_index` / `set_hint_text` / `item_rect` / `close_badge_rect`；**每张右上角有 `×` 角标**＝可见的移除入口，悬停变红并提示「移除第 N 张」）；页面另有 `clear_button(prefix)`（缩略图条右侧可见的「清空全部」，没图时禁用） |
...
- `loop.interval_seconds`：一轮跑完后的等待秒数（**沿用既有字段**，没有新增分钟字段）。
  页面上按**分钟**显示（1–720，设计稿的 `data-key` 叫 `loop_interval_minutes`），换算用
