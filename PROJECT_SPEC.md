...
4. **禁止收集或存储玩家账号、密码、token、设备指纹**；程序不得要求登录。
5. **禁止联网功能**：不自动更新、不上传日志、不调用任何远程服务。
6. **禁止多开批量打金、工作室用途**的加速设计（不做多实例协调、不做账号矩阵）。
7. **禁止未经用户确认就在真实模式下执行**：启动真实自动化前必须有明确 UI 确认 + 干跑预演。
8. **禁止**在无游戏窗口时静默执行、以及游戏窗口失焦后继续盲点（可配置为“失焦即暂停”）。

### 4.1 已取消的限制与风险声明（2026-09-15，用户决定）

以下原「永久禁止」条款**已按用户要求取消**，相关方案可在取得用户逐项明确授权后实施：

| 原条款 | 原位置 | 现状 |
|---|---|---|
| 禁止 DLL 注入 / 钩子注入 / 驱动级操作 | 本文件原 §4 第 2 条 | **已取消** |
| 禁止内置任何会显著增加封号风险的“过检测”技巧 | 本文件原 §4 第 8 条 | **已取消** |
| 禁止 DLL 注入 / 驱动 / 过检测（重复表述） | `AGENTS.md` 原 §3 红线 1 | **已取消** |

取消后**仍然必须遵守**的约束：

- **逐项授权**：任何驱动级 / 注入类 / 过检测类方案，必须先写入对应阶段的「本次只做什么」并取得用户明确确认，**禁止静默引入**；
- **可回退**：必须保证「不安装驱动、不做注入」的基础模式仍可独立运行；
- **风险声明**：README 与 GUI「关于」页必须如实声明下述风险。
...
3. **注入可被识别**：用户态注入的输入带 `LLMHF_INJECTED` 标记；驱动级注入同样可能被检测；
4. **封号概率显著上升**：此类方案一旦被判定违规，后果由用户自行承担（「个人自用、风险自负」）。
...
- **输入方式限制（已知风险）**：后台窗口消息对**以物理光标位置或 Raw Input/DirectInput 独占输入**决定点击位置的游戏无效（现象为“日志正常但点击落在真实光标处”）。遇到时按顺序排查：① 子窗口定位与同步投递是否生效；② 游戏是否需要前台焦点。若仍无效，**先向用户说明结论与可选方案**，不得擅自改为移动真实光标或注入真实输入。
- **政策变更带来的风险（2026-09-15）**：原禁止「DLL 注入 / 驱动级操作 / 过检测技巧」的条款已按用户要求取消（见 §4.1）。采用此类方案前必须完成逐项授权与风险声明，并优先评估风险更低的手段（窗口消息 → 用户态合成输入 → 驱动级注入）。
- **待用户决定的边界（当前仍生效）**：`SendInput` / `SetCursorPos` / `mouse_event` 等**用户态接管式接口当前仍被禁止**（见本节上一条与 `AGENTS.md` §1.6、§2）；若需启用，须由用户明确指示并同步修改本节、`AGENTS.md` 与 `PHASE_PROMPTS.md`。
...
| 输入模拟 | `ctypes` 调用 Windows 窗口消息：`SendMessageTimeout`（同步，带超时/防挂起）+ `PostMessage`（悬停提示），并用 `WindowFromPoint` 定位**子窗口**后投递（后台输入） | **不接管真实键鼠**：不移动真实光标、不抢占键盘焦点、不注入系统输入流；零第三方依赖，不引入 pyautogui |
