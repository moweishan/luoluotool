...
| ④ 绕过 ACE 反作弊检测（"过检测"） | **未授权** | 用户未回答该项，按**未授权**处理：禁止实现任何规避 ACE 检测/对抗反作弊的功能 |

**据此产生的硬约束（工程负责人执行）**：

- 只做**目标所需的最小动作**：注入阶段只加载 DLL、不挂钩、不改游戏代码；除注入本身必需的参数写入外不做任何内存/代码改动；
- **不做任何规避反作弊的对抗手段**（不隐藏模块、不抹除痕迹、不绕过完整性校验）；
...
4. 唯一能让该路线复活的手段是**绕过/对抗 ACE 反作弊检测（过检测）**——该项用户**未授权**，工程负责人**不予实现**；
5. 因此**该路线终止**：目标游戏的 Unity **IL2CPP + ACE + WeGame** 组合在系统保护层就拦住了注入与内存改写，反编译/hook 的工作量根本无从投入。点击坐标问题继续由 **Phase 5.2 对齐窗口通道**（实测达标：点准 + 真实光标不动 + 指针不消失 + 零注入）解决。
