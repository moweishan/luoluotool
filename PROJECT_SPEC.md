...
│   │   ├── dialogs/         # crop_dialog.py：框选截图生成模板（拖拽选区 → 存 assets/screenshots/）
...
│   ├── screenshots/         # 用 LuoLuoTool 截图/框选产出的图片（原始素材，不入库；原 anchors/ 并入此处）
│   └── icons/               # 静态资源：窗口图标等（入库）
...
- [ ] 框选生成模板：调试页「框选截图生成模板」→ 后台截图 → 弹窗拖拽框选 → 保存 `assets/screenshots/anchor_<时间戳>.png`（原 `assets/anchors/`，2026-09-20 并入截图目录）并**自动回填图片路径**；框选坐标即客户区坐标，用生成的模板识别必须命中同一坐标（有离线全流程验证）。
